...
const buttonClass = "w-full sm:w-auto min-h-[48px] px-6 md:px-8 whitespace-nowrap flex items-center justify-center gap-2 transition-all duration-300";
const imageClass = "w-full h-full object-contain p-4 transition-transform duration-500 will-change-transform group-hover:scale-105 select-none pointer-events-none";
...
const Index = () => {
  ...
  // Home Page Live Matrix Deduplication Logic
  const liveProducts = useMemo(() => {
    ...
    // 2. Overwrite with live DB lines
    dbProducts.forEach((d) => {
      ...
      if (normalName.includes("hyper no short") || normalName.includes("hyper-no")) {
        ...
        productFeaturedImage = "https://rjsmqpneamauasuoqzct.supabase.co/storage/v1/object/public/review-photos//98737dbc-d1ae-49e4-86bb-ddc9fc9f4565.png";
      } else if (normalName.includes("aminoshot") || normalName.includes("amino shot")) {
        productFeaturedImage = "https://rjsmqpneamauasuoqzct.supabase.co/storage/v1/object/public/review-photos//WhatsApp Image 2026-06-08 at 8.05.26 PM.jpeg";
    ...
    // 3. SECURE EXCLUSION FILTER: Blocks hidden junk items in the marquee
    const excludedProductNames = [
      "pure creatin", "pure creatine", "bcaa recover", "glutamine x", 
      "v-shot multivitamin", "daily multi", "myogenetix concentrate", 
      "ginseng extract", "amino shot caplets", "lean shot thermogenic",
      "super whey 2kg", "plasma mass 3kg"
    ];
    
    return result.filter(p => !excludedProductNames.includes(String(p?.name).toLowerCase().trim()));
  }, [dbProducts]);
  ...
  return (
    <div className="relative w-full overflow-x-hidden bg-white text-slate-900 antialiased selection:bg-slate-950 selection:text-white">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center">
            ...
            <div className="lg:col-span-7 space-y-6 text-center lg:text-left flex flex-col items-center lg:items-start order-2 lg:order-1">
              ...
              <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto pt-2">
                <Button asChild size="lg" className={`${buttonClass} bg-slate-900 text-white hover:bg-slate-800 font-black uppercase text-xs tracking-wider rounded-xl shadow-md h-12`}>
                  <Link to="/products">
                    Shop Now <ChevronRight className="h-4 w-4 shrink-0 stroke-[3]" />
                  </Link>
                </Button>

                <Button asChild size="lg" variant="outline" className={`${buttonClass} border-slate-900 text-slate-900 bg-transparent hover:bg-slate-50 font-black uppercase text-xs tracking-wider rounded-xl h-12`}>
            ...
            <div className="lg:col-span-5 order-1 lg:order-2 w-full flex justify-center">
              <div className="relative w-full max-w-[440px] aspect-[4/3] lg:aspect-square rounded-3xl border border-slate-200 overflow-hidden bg-slate-50 shadow-inner p-2 group">
                <video className="w-full h-full object-cover rounded-2xl opacity-90 transition-transform duration-700 group-hover:scale-102" src="/videos/hero.mp4" autoPlay muted loop playsInline />
      ...
      {/* ==================== SECTION 4: SECOND LAB GIF PROCESS BANNER ==================== */}
        ...
        <div className="relative flex w-full overflow-x-hidden select-none group/marquee border-y border-slate-100 py-4 bg-slate-50/30">
          <div className="flex flex-nowrap gap-6 animate-marquee whitespace-nowrap py-2 group-hover/marquee:[animation-play-state:paused]">
            ...
            {liveProducts.map((p, i) => {
              const routeParam = p.id || p.slug;
              return (
                <Link key={`marquee-a-${routeParam}-${i}`} to={`/products/${routeParam}`} className="group block w-[240px] md:w-[280px] shrink-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1.5 hover:border-slate-400 hover:shadow-md">
                  ...
                  <div className="p-5 text-left whitespace-normal space-y-1.5">
                    <p className="text-[9px] font-black uppercase tracking-[0.25em] text-blue-600">{p.category}</p>
                    <h3 className="line-clamp-1 text-base font-black tracking-tight text-slate-800 group-hover:text-slate-900">{p.name}</h3>
                    <p className="line-clamp-2 text-xs text-slate-400 font-medium leading-relaxed h-8">{p.tagline}</p>
            ...
            {liveProducts.map((p, i) => {
              const routeParam = p.id || p.slug;
              return (
                <Link key={`marquee-b-${routeParam}-${i}`} to={`/products/${routeParam}`} className="group block w-[240px] md:w-[280px] shrink-0 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1.5 hover:border-slate-400 hover:shadow-md">
                  <div className="aspect-[4/5] overflow-hidden bg-slate-50 relative flex items-center justify-center border-b border-slate-100">
                    <img src={p.image} alt={p.name} loading="lazy" className={imageClass} />
                  </div>
                  <div className="p-5 text-left whitespace-normal space-y-1.5">
