...
// Utility to ensure Razorpay loads correctly
...
const Checkout = () => {
  ...
  const placeOrder = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // In a full production app, this order ID should ideally be generated by your backend/Edge Function.
      const fakeRzpOrderId = `order_LIVE_${Math.random().toString(36).slice(2, 12)}`;
      ...
      // Razorpay live checkout configuration
      const options = {
        key: import.meta.env.VITE_RAZORPAY_KEY_ID || "rzp_live_mrY8DTan2XlmdQ", // Live Razorpay Key
  ...
  if (paid) return (
    <>
      <PageHero eyebrow="Confirmed" title="Payment successful" subtitle="Your payment has been captured and your order is being processed." />
      ...
      <section className="py-16">
        <form className="container grid lg:grid-cols-3 gap-10" onSubmit={placeOrder}>
          ...
          <aside className="bg-card border border-white/10 rounded-xl p-6 h-fit space-y-4">
            ...
            <Button disabled={busy} type="submit" size="lg" className="w-full bg-primary hover:bg-primary/90 shadow-glow">
              {busy ? "Processing payment…" : "Pay Securely"}
            </Button>
            <p className="text-xs text-white/40 flex items-center justify-center gap-1.5 mt-2">
              <ShieldCheck className="h-3 w-3 text-emerald-500" /> 256-bit secure payment
            </p>
          </aside>
        </form>
      </section>
