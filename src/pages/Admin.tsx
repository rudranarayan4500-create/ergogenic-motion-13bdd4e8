...
const Section = ({ children }: { children: React.ReactNode }) => (
  <div className="bg-card border border-white/10 rounded-xl p-6">{children}</div>
);
...
export default function Admin() {
  ...
  const [products, setProducts] = useState<any[]>([]); // Kept for the stats counter
          ...
          <div className="flex justify-between items-center">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 flex-1 mr-4">
              {[
                ...
                <div key={s.l} className="bg-card border border-white/10 rounded-xl p-4">
                      ...
                      {profiles.map(p => (
                        <tr key={p.id} className="border-b border-white/5">
                          ...
                          <td className="text-right space-x-2">
                            <Button size="sm" variant="outline" className="border-white/15" onClick={() => blockUser(p.id, !p.blocked)}>
                  ...
                  <form onSubmit={saveProduct} className="space-y-4 text-sm">
                    {["slug", "name", "tagline", "category"].map(k => (
                      <div key={k}>
                        ...
                        <Input name={k} required={k === "slug" || k === "name" || k === "category"} className="mt-1 bg-background border-white/15" />
                      </div>
                    ))}
                    <div>
                      <Label>Image</Label>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <Label>Price</Label>
                        <Input name="price" type="number" required className="mt-1 bg-background border-white/15" />
                      </div>
                      <div>
                        <Label>MRP</Label>
                        <Input name="mrp" type="number" className="mt-1 bg-background border-white/15" />
                      </div>
                    </div>
                    <div>
                      <Label>Description</Label>
                      <Textarea name="description" rows={4} className="mt-1 bg-background border-white/15" />
                    </div>
                    <Button type="submit" className="bg-primary hover:bg-primary/90 w-full mt-2">Save Product</Button>
                  </form>
                </Section>
              </div>
            </TabsContent>
                    ...
                    <div key={r.id} className="p-4 bg-background/40 border border-white/5 rounded-lg">
                      <div className="flex justify-between items-start gap-3">
                        ...
                        <div className="flex flex-col gap-1">
                          ...
                          <Button size="icon" variant="ghost" onClick={() => toggleReview(r.id, !r.approved)}>
            ...
            <TabsContent value="settings">
              <Section>
                <div className="space-y-4 max-w-md">
                  <div>
                    <Label>Admin email (lock to one account)</Label>
                    <Input value={settings.admin_email || ""} onChange={(e) => setSettings({ ...settings, admin_email: e.target.value })} className="mt-1.5 bg-background border-white/15" />
                  </div>
                  <div>
                    <Label>Admin secret code</Label>
                    <Input value={settings.secret_code || ""} onChange={(e) => setSettings({ ...settings, secret_code: e.target.value })} className="mt-1.5 bg-background border-white/15" />
                  </div>
